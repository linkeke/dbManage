...
dc.printers.filter = function (filter) {
    ...
    if (typeof filter !== 'undefined' && filter !== null) {
        if (filter instanceof Array) {
            if (filter.length >= 2) {
                var thisBegin = new Date(filter[0].getTime());      // Mark edit: fix beginDate not inclusive
                thisBegin.setDate(thisBegin.getDate() + 1);
                var thisEnd = new Date(filter[1].getTime());
                ...
                if (thisBegin > thisEnd)
                  ...
                  s = '[' + dc.utils.printSingleValue(thisBegin) + ' -> ' + dc.utils.printSingleValue(thisEnd) + ']';
...
dc.stackMixin = function (_chart) {
    ...
    function prepareValues (layer, layerIdx) {
        ...
        //layer.values = layer.values.filter(domainFilter());  // Mark edit - don't filter until flattenStack, for better transition effect
        return layer.values;
    }
    ...
    function flattenStack () {
        var valueses = _chart.data().map(function (layer) { return layer.values.filter(domainFilter()); });   // Mark edit - filter here
...
dc.lineChart = function (parent, chartGroup) {
    ...
    function drawDots (chartBody, layers) {
        //if (_chart.xyTipsOn() === 'always' || (!_chart.brushOn() && _chart.xyTipsOn())) {  // Mark edit
